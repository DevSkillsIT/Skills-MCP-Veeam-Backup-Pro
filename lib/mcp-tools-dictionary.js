 ...
 * Define schemas JSON para 16 ferramentas divididas em 3 categorias:
...
export const MCP_TOOLS_DICTIONARY = {
  ...
  "get-running-sessions": {
    ...
    description: "Lista TODAS as sessions em execução (backup jobs + system tasks). Retorna separado em duas categorias: backupJobs (backups reais como BackupJob, ReplicaJob, BackupCopyJob) e systemTasks (tarefas de manutenção como MalwareDetection, SureBackup, Restore). Use get-running-backup-jobs para ver apenas backup jobs.",
    inputSchema: {
      type: "object",
      properties: {
        limit: {
  ...
  "get-running-backup-jobs": {
    name: "get-running-backup-jobs",
    description: "Lista APENAS backup jobs em execução (BackupJob, ReplicaJob, BackupCopyJob, EpAgentBackup). EXCLUI automaticamente system tasks como MalwareDetection, SureBackup, Restore, etc. Use esta tool quando precisar ver apenas backups reais rodando.",
    inputSchema: {
      type: "object",
      properties: {
        limit: {
          type: "number",
          description: "Máximo de backup jobs a retornar (padrão: 100)",
          default: 100,
          minimum: 1,
          maximum: 1000
        }
